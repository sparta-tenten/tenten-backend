...
public class MenuService {
    ...
    //검색/정렬/페이징
    public Page<MenuDto> searchMenus(UUID storeId, String keyword, String sortBy, String direction,
        int page, int size) {
        Sort sort = direction.equalsIgnoreCase("desc") ? Sort.by(sortBy).descending()
            : Sort.by(sortBy).ascending();
        Pageable pageable = PageRequest.of(page, size, sort);

        Page<Menu> menuPage = menuRepository.findByStoreIdAndIsDeletedFalseAndNameContainingIgnoreCaseOrCategoryContainingIgnoreCase(
